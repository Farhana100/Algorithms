...
public:
    ...
    bool bipartite();
    bool bipartite_dfs(int src, int p);
...
bool Graph::addEdge(int u, int v, int w = 1)
{
...
bool Graph::bipartite_dfs(int src, int p)
{
    visited[src] = true;
    int sz = adj_list[src].size();
    int col = !colour[src];

    for(int i = 0;i < sz; i++){
        int v = adj_list[src][i].getTo();
        if(colour[v] != -1){
            if(col != colour[v]){
                return false;
            }
        ...
        colour[v] = col;
        if(!bipartite_dfs(v, src)) return false;
    }
...
void Graph::bipartite_matching()
{
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 0; j < adj_list[i].size(); j++){
            G.addEdge(i, adj_list[i][j].getTo(), 1);
        }
    }
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 0; j < G.adj_list[i].size(); j++){
            if(G.adj_list[i][j].getflow()){
                fout << i << ' ' << G.adj_list[i][j].getTo() << endl;
            }
        }
    }
